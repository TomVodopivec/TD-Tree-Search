...
Player_AI_UCT::UCTnode* Player_AI_UCT::UCT_AMAF_Tree_Policy(UCTnode* root, int simulation_number)
{
...
*/
Player_AI_UCT::UCTnode* Player_AI_UCT::UCT_AMAF_Tree_Policy_Best_Child(UCTnode* parent, double param_C) {
  ...
  for (int i = 0; i < parent->number_allowed_actions; i++) {
    ...
    if (parent->children[i] != NULL) {
      ...
      //DEBUG
#if(TOMPlayer_AI_UCT_DEBUG_TRACE_ALL_UCT)
...
*/
double* Player_AI_UCT::UCT_AMAF_Default_Policy(int simulation_number) {
...
*/
void Player_AI_UCT::UCT_AMAF_Backup(UCTnode* leaf, double* rewards, int simulation_number) {
  ...
  //DEBUG AMAF
#if(TOMPlayer_AI_UCT_DEBUG_BACKUP_SHOW_AMAFTAGS)
...
*/
Player_AI_UCT::UCTnode* Player_AI_UCT::UCT_Tree_Node_Init(int action, UCTnode* parent, const bool rootNode)
{
  UCTnode* node;
