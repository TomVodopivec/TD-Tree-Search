...
// -- end - Q-value parameter search and approximation -- //
	...
	//printf("\n--- Rew % 1.0f   P %2.0f   trace %5.3f   ", (sim_feedback_scores_avg[belonging_player] - 0.5), sim_feedback_plys_avg, Q_gammatrace*set_QVALUE_param_gamma);
	...
	//double TD_leaf_state_value = QVALUE_param_initVal;	//TDmin (TD-UCT WR)
	//TDfourth[belonging_player] = 0;
	//TDfourth[1-belonging_player] = 0;

	////used for CIG14 TD - UCT paper
	////TDfourth[belonging_player] = ((set_QVALUE_param_gamma*TD_next_state_value - TD_leaf_state_value));
	////TDfourth[belonging_player] = (-(set_QVALUE_param_gamma*TD_next_state_value - TD_leaf_state_value));

	//correction for TDMCTS paper for TCIAIG journal
	TDfourth[belonging_player] = ((TD_next_state_value - TD_leaf_state_value));
	TDfourth[1-belonging_player] = (-(TD_next_state_value - TD_leaf_state_value));
	...
	for(int i = 0; i < UCTtree_treePolicy_lastDepth+1; i++){
		...
		////printf(" Qval(prej)=%5.3f , Q_trace=%5.3f , a=%5.3f , R=%1f , ", currentNode->Q_value, Q_trace, set_QVALUE_param_alpha, sim_feedback_scores_avg[belonging_player]);
		...
		//set_QVALUE_param_alpha = 1.0 / currentNode->visits;
		//printf("--- Qval(prej)=%5.3f , a=%5.3f , e*R=%1.3f ,", currentNode->Q_value +0.5, set_QVALUE_param_alpha, TDfourth[belonging_player]);
...
double Player_AI_UCT_TomTest::Neural_Network_Threshold_Function(double input_weight_sum, const int function_type)
{
	...
	if (function_type == TOMPLAYER_AI_UCT_TOMTEST_NN_FUNCTION_TYPES_SIGMOID_CENTER_ZERO){
		return (1.0 / (1.0 + exp(-input_weight_sum)) - 0.5);

	}
	else if (function_type == TOMPLAYER_AI_UCT_TOMTEST_NN_FUNCTION_TYPES_SIGMOID){
		return (1.0 / (1.0 + exp(-input_weight_sum)));

	}
	else if (function_type == TOMPLAYER_AI_UCT_TOMTEST_NN_FUNCTION_TYPES_STEP){
		return ((double)(input_weight_sum > 0.0));

	}
	else if (function_type == TOMPLAYER_AI_UCT_TOMTEST_NN_FUNCTION_TYPES_LINEAR){
